...
class YouTubeDownloder:
    def __init__(self):
        self.url = st.text_input(str(input("Enter the url of video : ")))
    ...
    def showStreams(self):
        ...
        for stream in self.youtube.streams:
            # Check if fps attribute exists, otherwise use "N/A"
            fps = getattr(stream, 'fps', 'N/A')
            print(
                "{0} => resolution:{1}/fps:{2}/type:{3}".format(
                    self.streamNo, stream.resolution, fps, stream.type
                )
            )
            self.streamNo += 1
        self.chooseStream()
    ...
    def getPermisionToContinue(self):
        # Check if fps attribute exists, otherwise use "N/A"
        fps = getattr(self.stream, 'fps', 'N/A')
        print(
            "\n title : {0} \n author : {1} \n size : {2:.2f}MB \n resolution : {3} \n fps : {4} \n ".format(
                self.youtube.title,
                self.youtube.author,
